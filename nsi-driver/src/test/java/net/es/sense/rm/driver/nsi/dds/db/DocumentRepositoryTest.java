...
public class DocumentRepositoryTest {
  ...
  public void verify() throws JAXBException, IOException, DatatypeConfigurationException {
    ...
    Assert.assertEquals(3, documents.count());
  ...
  @Test
  public void findNewer() throws DatatypeConfigurationException, JAXBException, IOException {
    // Set up test data.
    buildDatabase();

    long searchTime = System.currentTimeMillis();
    Iterable<Document> findNewer = documents.findNewer(searchTime);
    int count = 0;
    for (Document doc : findNewer) {
      count++;
    }
    ...
    findOne.setLastDiscovered(now);
    documents.save(findOne);
    findNewer = documents.findNewer(searchTime);
    for (Document doc : findNewer) {
      count++;
    }
  ...
  @Test
  public void findByNsa() throws JAXBException, IOException, DatatypeConfigurationException {
    // Set up test data.
    buildDatabase();

    Collection<Document> results = Lists.newArrayList(documents.findByNsa("urn:ogf:network:es.net:2013:nsa"));
    Assert.assertEquals(2, results.size());

    results = Lists.newArrayList(documents.findByNsa("urn:ogf:network:poopie.net:2013:poop"));
    Assert.assertEquals(0, results.size());
  }

  @Test
  public void findByType() throws JAXBException, IOException, DatatypeConfigurationException {
    // Set up test data.
    buildDatabase();

    Collection<Document> results = Lists.newArrayList(documents.findByType("vnd.ogf.nsi.topology.v2+xml"));
    Assert.assertEquals(1, results.size());

    results = Lists.newArrayList(documents.findByType("vnd.ogf.nsi.nsa.v1+xml"));
    Assert.assertEquals(2, results.size());

    results = Lists.newArrayList(documents.findByNsa("vnd.ogf.nsi.poop.v1+xml"));
    Assert.assertEquals(0, results.size());
  }

  @Test
  public void findExpired() throws JAXBException, IOException, DatatypeConfigurationException {
    // Set up test data.
    buildDatabase();
    Date expires = XmlUtilities.xmlGregorianCalendarToDate(XmlUtilities.xmlGregorianCalendar("2017-12-05T10:15:05.536Z"));
    Collection<Document> expired = Lists.newArrayList(documents.findExpired(expires.getTime()));
    ...
    expired = Lists.newArrayList(documents.findAll());
    Assert.assertEquals(1, expired.size());
  }

  @Test
  public void findByNsaAndType() throws JAXBException, IOException, DatatypeConfigurationException {
    // Set up test data.
    buildDatabase();

    Collection<Document> results = Lists.newArrayList(documents.findByNsaAndType("urn:ogf:network:es.net:2013:nsa", "vnd.ogf.nsi.topology.v2+xml"));
    Assert.assertEquals(1, results.size());
  }

  @Test
  public void findByNsaAndTypeAndDocumentId() throws JAXBException, IOException, DatatypeConfigurationException {
    // Set up test data.
    buildDatabase();

    Collection<Document> results = Lists.newArrayList(documents.findByNsaAndTypeAndDocumentId("urn:ogf:network:es.net:2013:nsa", "vnd.ogf.nsi.topology.v2+xml", "urn:ogf:network:es.net:2013:"));
    Assert.assertEquals(1, results.size());
  }
